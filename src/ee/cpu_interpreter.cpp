...
{
    s8 val = virtual_read<s8>(gpr[rs].s32() + imm);
...
{
    u8 val = virtual_read<s8>(gpr[rs].s32() + imm);
    if (!exception_occurred) {
        gpr.set(rt, val);
    }
}
...
{
    s64 val = virtual_read<s64>(gpr[rs].s32() + imm);
    if (!exception_occurred) {
        gpr.set(rt, val);
    }
}
...
{
    s16 val = virtual_read<s16>(gpr[rs].s32() + imm);
    if (!exception_occurred) {
        gpr.set(rt, val);
    }
}
...
{
    u16 val = virtual_read<s16>(gpr[rs].s32() + imm);
    if (!exception_occurred) {
        gpr.set(rt, val);
    }
}
...
{
    s32 val = virtual_read<s32>(gpr[rs].s32() + imm);
    if (!exception_occurred) {
        gpr.set(rt, val);
    }
}
...
{
    u32 val = virtual_read<s32>(gpr[rs].s32() + imm);
    if (!exception_occurred) {
        gpr.set(rt, val);
    }
}
